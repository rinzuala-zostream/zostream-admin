    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors text-gray-900 dark:text-gray-100">
      ...
      <main class="max-w-6xl mx-auto px-4 py-8">
            ...
            <form @submit.prevent="createPoll" class="space-y-4">
              <div>
                ...
                <input
                  ...
                  class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  required
                />
              </div>
              ...
              <div class="flex items-center">
                <label class="flex items-center cursor-pointer">
                  <div class="relative">
                    <input type="checkbox" v-model="newPoll.status" class="sr-only" />
                    <div class="w-10 h-4 bg-gray-300 dark:bg-gray-700 rounded-full shadow-inner"></div>
                    <div
                      class="dot absolute w-6 h-6 rounded-full shadow -left-1 -top-1 transition"
                      :class="{'bg-blue-500 transform translate-x-6': newPoll.status, 'bg-gray-400': !newPoll.status}"
                    ></div>
                  </div>
                  <div class="ml-3 font-medium">Active Poll</div>
                </label>
              </div>
              ...
              <div class="space-y-3">
                ...
                <div v-for="(option, index) in newPoll.options" :key="index" class="p-4 rounded-lg bg-gray-100 dark:bg-gray-700 transition-all">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500"
                        required
                      />
                    </div>
                    <div>
                  ...
                  <button
                    ...
                    class="mt-2 text-sm flex items-center text-red-500 hover:text-red-600 transition-colors"
                  >
              ...
              <button
                type="submit"
                class="w-full mt-4 py-3 px-6 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold rounded-lg shadow-md transition-all transform hover:scale-[1.02]"
              >
                Create Poll
              </button>
            </form>
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex items-center gap-2">
                  <span
                    ...
                    :class="{
                      'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400': poll.data.status,
                      'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400': !poll.data.status
                    }"
                  >
                  ...
                  <button
                    ...
                    class="p-2 rounded-full hover:bg-red-500/10 text-red-500 hover:text-red-600 transition-colors"
                    title="Delete Poll"
                  >
                ...
                <div v-for="(label, key) in poll.data.options" :key="key" class="p-4 rounded-lg bg-gray-100 dark:bg-gray-700 transition-all">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                      <div class="relative">
                        ...
                        <div v-if="poll.data.votes?.[key]?.length"
                             class="absolute -top-2 -right-2 bg-blue-500 text-white text-xs font-bold rounded-full h-5 w-8 flex items-center justify-center">
                          {{ poll.data.votes?.[key]?.length }}
                        </div>
                      </div>
                      <span class="font-medium">{{ label }}</span>
                    </div>
                    <button
                      @click="toggleVoters(poll.id, key)"
                      class="text-sm flex items-center gap-1 px-3 py-1 rounded-full"
                    >
                      <span>Voters</span>
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform"
                           :class="{'rotate-180': expandedVoters[`${poll.id}-${key}`]}"
                  ...
                  <div v-if="expandedVoters[`${poll.id}-${key}`]" class="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600">
                    ...
                    <div v-if="poll.data.votes?.[key]?.length" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                      <div v-for="(uid, index) in poll.data.votes?.[key]" :key="index"
                           class="text-sm px-3 py-1 rounded-full flex items-center gap-2 bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-100">
        ...
        <transition name="fade">
          <div v-if="message"
               class="fixed bottom-4 right-4 px-6 py-3 rounded-lg shadow-lg flex items-center gap-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
