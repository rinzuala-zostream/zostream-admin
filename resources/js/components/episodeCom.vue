<template>
    <div class="max-w-10xl px-4 sm:px-6 lg:px-8 py-8">
        <div
            ...
            <!-- Form Header -->
            <div
                class="bg-gradient-to-r from-blue-500 to-indigo-600 dark:from-blue-600 dark:to-indigo-700 px-6 py-4 sm:px-8">
                <div class="flex items-center justify-between">
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/20 text-white/90">
                        TV Series
                    </span>
                </div>
            </div>
            ...
            <div class="px-6 py-6 sm:px-8 sm:py-8">
                <form @submit.prevent="submitForm" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div v-if="form.img" class="mt-2">
                                    ...
                                    <img :src="form.img" alt="Cover preview"
                                        class="mt-1 h-20 w-auto rounded border border-gray-200 dark:border-gray-700 object-cover">
                                </div>
                            </div>
                    ...
                    <div class="search-section pt-8 border-t border-gray-200 dark:border-gray-800">
                        ...
                        <div v-if="selectedItemInfoText"
                            class="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 flex justify-between items-center">
                            ...
                            <button @click="clearSelectedItem" type="button"
                                class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 text-xs font-medium p-1 rounded hover:bg-blue-100 dark:hover:bg-blue-800/30 transition-colors">
                                Clear
                            </button>
                        </div>
                        ...
                        <div class="mb-6">
                            <div class="flex flex-col sm:flex-row gap-4 mb-3">
                                <div class="relative flex-grow">
                                    ...
                                    <input type="text" v-model="search" placeholder="Search by title to find Show..."
                            ...
                            <div v-if="!searchLoading">
                                <div v-if="hasSearchedAtLeastOnce && movies.length === 0 && search.trim() !== ''"
                                    class="text-center py-4 text-gray-500 dark:text-gray-400">
                                    No items found matching "{{ search }}"
                                </div>
                                <!-- === MODIFIED: Search Results List with Season Drill-down === -->
                                <ul v-if="movies.length > 0"
                                    class="space-y-3 max-h-96 overflow-y-auto mt-4 border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30 custom-scrollbar">
                                    <li v-for="movie in filteredMovies" :key="movie.id"
                                        ...
                                        <button type="button" class="w-full text-left" @click="toggleSeasons(movie)">
                                            <div
                                                ...
                                                <div class="flex items-center space-x-3 min-w-0">
                                                    ...
                                                    <div class="min-w-0">
                                                        <p
                                                            class="font-medium truncate text-gray-800 dark:text-gray-100">
                                                            {{ movie.title }}</p>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                                                            Num: {{ movie.num }} • ID: {{ movie.id }}</p>
                                                    </div>
                                                </div>
                                                <svg class="w-5 h-5 text-gray-400 dark:text-gray-500 transition-transform duration-300"
                                                    :class="{ 'rotate-90': movie.showSeasons }"
                                        ...
                                        <div v-if="movie.showSeasons"
                                            class="bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700">
                                            <div v-if="movie.seasonsLoading"
                                                class="text-center py-4 text-sm text-gray-500">Loading seasons...</div>
                                            <div v-else-if="!movie.seasons || movie.seasons.length === 0"
                                                class="text-center py-4 text-sm text-gray-500">No seasons found.</div>
                                            <ul v-else class="py-2 px-3 space-y-1">
                                                <li v-for="season in movie.seasons" :key="season.id">
                                                    <button type="button"
                                                        class="w-full text-left p-2 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                                                        @click="selectSeason(movie, season)">
                                                        <div class="flex items-center space-x-2">
                                                            <svg class="w-4 h-4 text-gray-500 flex-shrink-0" fill="none"
                                                                stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2" d="M3 10h18M3 14h18m-9-4v8"></path>
                                                            </svg>
                                                            <span
                                                                class="text-sm font-medium text-gray-700 dark:text-gray-200">{{season.txt }}</span>
                                                            <span class="text-xs text-gray-400 dark:text-gray-500">(ID:
                                                                {{ season.id }})</span>
                                                        </div>
                                                    </button>
                                                </li>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <label v-for="(label, key) in booleanFields" :key="key"
                                ...
                                <div class="flex items-center h-5">
                                    <input type="checkbox" :id="`checkbox-${key}`" v-model="form[key]"
                                        class="sr-only peer">
                    ...
                    <div class="attributes-section pt-8 border-t border-gray-200 dark:border-gray-800">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <label
                                class="checkbox-label flex items-start space-x-3 cursor-pointer p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" :id="checkbox" v-model="form.notification"
                                        class="sr-only peer">
                                    <span class="custom-checkbox w-5 h-5 rounded border flex items-center justify-center
                               border-gray-400 dark:border-gray-600 bg-white dark:bg-gray-800
                               group-hover:border-gray-500 dark:group-hover:border-gray-500
                    ...
                    <div class="pt-8 flex flex-col sm:flex-row justify-end space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        <button type="submit"
                            ...
                            <svg v-if="!formSubmitting" class="w-5 h-5" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800': message.startsWith('Failed')
                        ...
                        <svg v-if="!message.startsWith('Failed')"
                            class="h-5 w-5 mr-2 text-green-500 dark:text-green-400 flex-shrink-0" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <svg v-else class="h-5 w-5 mr-2 text-red-500 dark:text-red-400 flex-shrink-0" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
.custom-scrollbar::-webkit-scrollbar-track {
    ...
    /* Or use a background from your theme e.g., bg-gray-100 dark:bg-gray-800 */
}
