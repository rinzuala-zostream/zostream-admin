    ...
    >
      <div v-if="isVisible" class="fixed inset-0 z-50 bg-gray-900/80 backdrop-blur-sm" @click.self="$emit('close')">
        <div class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 sm:pt-24 overflow-y-auto">
          ...
          >
            <div v-if="isVisible" class="relative bg-white dark:bg-gray-800 w-full max-w-4xl rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <!-- Modal Header -->
              <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm z-10">
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100">Edit Advertisement</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update your advertisement details</p>
                  </div>
                </div>
                <button @click="$emit('close')" type="button" class="p-2 rounded-full text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200" aria-label="Close">
              ...
              <!-- Modal Body -->
              <div class="p-7 max-h-[65vh] overflow-y-auto">
                <!-- Loading State -->
                <div v-if="isLoading" class="flex justify-center items-center h-96">
                  <div class="text-center">
                    <svg class="animate-spin mx-auto h-12 w-12 text-orange-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                    </svg>
                    <p class="mt-4 text-gray-600 dark:text-gray-400">Loading Ad Data...</p>
                  </div>
                </div>

                <!-- Error State -->
                <div v-else-if="error" class="text-center p-8 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
                ...
                <!-- Main Form -->
                <form v-else-if="formData" @submit.prevent="handleSubmit" class="space-y-8">
                  ...
                  <div class="space-y-6">
                    ...
                    <div>
                      <label for="ad_name_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ad Name <span class="text-red-500">*</span></label>
                      <input v-model="formData.ads_name" id="ad_name_edit" type="text" required 
                    ...
                    <div>
                      <label for="ad_description_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
                      <textarea v-model="formData.description" id="ad_description_edit" rows="4" 
                                class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors duration-200" 
                                placeholder="A detailed description of your advertisement..."></textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                        <label for="ad_period_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Period (days) <span class="text-red-500">*</span></label>
                        <input v-model.number="formData.period" id="ad_period_edit" type="number" required 
                               class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors duration-200" 
                               placeholder="e.g., 7">
                      </div>
                      <div>
                        <label for="ad_type_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Type</label>
                        <select v-model="formData.type" id="ad_type_edit" 
                  ...
                  <div class="space-y-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div>
                      <label for="ad_feature_img_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Feature Image URL <span class="text-red-500">*</span></label>
                      <input v-model="formData.feature_img" id="ad_feature_img_edit" type="url" required 
                             class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors duration-200" 
                    ...
                    <div>
                      <label for="ad_video_url_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Video URL</label>
                      <input v-model="formData.video_url" id="ad_video_url_edit" type="url" 
                             class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors duration-200" 
                  ...
                  <div class="space-y-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex items-center">
                      <div class="h-6 w-1.5 bg-gradient-to-b from-orange-500 to-orange-600 rounded-full mr-3"></div>
                      <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Additional Images</h4>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div v-for="i in 4" :key="i" class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-700/70">
                        <label :for="`ad_img_${i}_edit`" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Image {{ i }} URL</label>
                        <input v-model="formData[`img${i}`]" :id="`ad_img_${i}_edit`" type="url" 
                  ...
                  <!-- Modal Footer -->
                  <div class="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700 space-y-4 sm:space-y-0 sm:space-x-3">
...
const props = defineProps({
  ...
  adId: { type: [String, Number], default: null } // This will be the `num`
});
...
// Fetches the specific ad's data to populate the form
const fetchAdData = async (id) => {
  isLoading.value = true;
  error.value = null;
  console.log("Fetching ad with ID:", id);
  try {
    const response = await axios.get(route('proxy.get', { endpoint: `ads/${id}` }));
    // Based on your API, a single ad might be returned directly or nested in a `data` object.
    // This code handles both possibilities.
    const adData = response.data.data || response.data;
    if (adData && typeof adData === 'object') {
      formData.value = adData;
    } else {
      throw new Error("Ad not found or invalid data format");
    }
  } catch (err) {
    error.value = "Failed to load ad data. The ad may not exist or the API is unavailable.";
...
const handleSubmit = async () => {
  ...
  try {
    await axios.put(route('proxy.put'), formData.value, { 
        params: { endpoint: `ads/${props.adId}` } 
    });
    toast.success("Ad updated successfully!");
    emit('saved'); // Notify parent to close and refresh
  } catch (err) {
    submissionError.value = err.response?.data?.message || "Update failed. Please check the details and try again.";
...
// Watch for the modal to become visible
watch(() => props.isVisible, (newValue) => {
  if (newValue && props.adId) {
    fetchAdData(props.adId);
  } else if (!newValue) {
